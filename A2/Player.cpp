...
namespace ducks
{
	...
	Action Player::shoot(const GameState& pState, const Deadline& pDue)
	{
		...
		// 2. Don't even bother with a dead bird
		...
		// ******************************************************************************************
		// Check the sequence of the movement of the bird
		...
		// ******************************************************************************************
		// Get the bird's next move with the alpha-forward algorithm
		...
		if (birdSpecies != SPECIES_BLACK_STORK && testHMM.boolConverge == 1 && move != -1) {
			cerr << "Shooting" << "   bird: " << birdIndex << ", numbirds: " << pState.getBird(birdIndex).getSeqLength() << endl;
			numOfShots++;
